class ResearchesController < ApplicationController
  ...
  # GET /researches.json
  def index
    @researches = Research.search(params[:search]).paginate(page: params[:page], per_page: 10).order('created_at DESC')
  ...
  def update
    @research.assign_attributes(research_params)
    if  @research.changed?
           @research.edit_by = current_user.email
    end
    respond_to do |format|
  ...
  private
    ...
    def research_params
      params.require(:research).permit(:user_id, :title, :status, :research, :docs, :edit_by)
    end
end
